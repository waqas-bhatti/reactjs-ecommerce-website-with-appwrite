...
function CheckoutForm() {
  ...
  const handleConfirmOrder = async () => {
    ...
    setLoading(true);
    setIsOrderConfirmed(true); // Mark order as confirmed in the UI
    ...
    try {
      ...
      // Save the order to the backend (Appwrite)
      ...
      await Promise.all(promises); // Ensure all order data is saved

      // Now we reuse handleRemoveCart to remove each item from the cart
      for (const item of selectedItems) {
        await handleRemoveCart(item.id); // Use the existing function to remove items
      }

      // After clearing the cart, reset the local state
      setSelectedItems([]); // Clear items from local state

      setTimeout(() => {
        setLoading(false); // Stop the loading spinner
        navigate("/order-page", { state: { orderId: orderId } }); // Navigate to order confirmation page
      }, 2000);
    } catch (error) {
